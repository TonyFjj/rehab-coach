...
static int g_ref_count = 1;

const char* get_assessment_log_path(void)
{
    return imu_get_assessment_log_path();
}
              ...
              double fc, double fs)
{
    ...
    /* 反转 */
    double* rev = (double*)calloc(len, sizeof(double));
    if (!rev) {
...
static void update_filtered_ang_vel_mag(IMU_Record* data, int len)
{
    if (len <= 0) return;

    double* wx = (double*)malloc(len * sizeof(double));
    double* wy = (double*)malloc(len * sizeof(double));
    double* wz = (double*)malloc(len * sizeof(double));
    ...
    filtfilt(wx, wxf, len, 10.0, SAMPLE_RATE);
    filtfilt(wy, wyf, len, 10.0, SAMPLE_RATE);
    filtfilt(wz, wzf, len, 10.0, SAMPLE_RATE);

    for (int i = 0; i < len; i++) {
        data[i].ang_vel_mag = sqrt(wxf[i] * wxf[i] +
                                   wyf[i] * wyf[i] +
                                   wzf[i] * wzf[i]);
    }
...
static double percentile_array(const double* values, int len, double pct)
{
    if (len <= 0) return 0.0;
    ...
    memcpy(tmp, values, len * sizeof(double));
    qsort(tmp, len, sizeof(double), compare_double_asc);

    if (pct < 0.0) pct = 0.0;
    if (pct > 1.0) pct = 1.0;
    int idx = (int)round(pct * (len - 1));
    double result = tmp[idx];
    free(tmp);
    return result;
}

static double percentile_ang_vel_mag(const IMU_Record* data, int start, int end, double pct)
{
    if (start < 0) start = 0;
    ...
    if (!vals) {
        ...
        for (int i = start; i < end; i++) {
            if (data[i].ang_vel_mag > max_v) max_v = data[i].ang_vel_mag;
        }
        return max_v;
    }

    for (int i = 0; i < len; i++) {
        vals[i] = data[start + i].ang_vel_mag;
    }

    double result = percentile_array(vals, len, pct);
    free(vals);
    return result;
}

static double detrended_ang_vel_std(const IMU_Record* data, int start, int end)
{
    int n = end - start;
    if (n < 3) return 0.0;
    ...
    for (int i = 0; i < n; i++) {
        ...
        double y = data[start + i].ang_vel_mag;
        sum_t += t;
        sum_y += y;
        sum_tt += t * t;
        sum_ty += t * y;
    ...
    double slope = (fabs(denom) > 1e-10) ?
        (n * sum_ty - sum_t * sum_y) / denom : 0.0;
    ...
    for (int i = 0; i < n; i++) {
        ...
        double residual = data[start + i].ang_vel_mag - trend;
...
int read_imu_csv(const char* filename, IMU_Record* data)
{
    ...
    if (is_physical) {
        ...
        int ref_s = (row > PREP_FRAMES) ? PREP_FRAMES : row - 1;
        int ref_c = 1;
        ...
        if (ax_raw && ay_raw && az_raw && ax_f && ay_f && az_f) {
            ...
            /* 内存不足, 回退到简单pitch方法 */
            double rest_pitch = data[ref_s].pitch;
            for (int i = 0; i < row; i++) {
        ...
        update_filtered_ang_vel_mag(data, row);
     ...
     * 稳定判据: 加速度幅值标准差 < 均值的20% */
    int ref_start = (row > PREP_FRAMES) ? PREP_FRAMES : row - 1;
    int ref_count = 1;
    int bias_start = MAX(0, ref_start - MIN_REST_FRAMES);
    int bias_count = ref_start - bias_start;
    if (bias_count < 1) {
        bias_start = ref_start;
        bias_count = 1;
    }
    int stable_found = 0;
    ...
    #define CHECK_STABLE_WINDOW(start) do { \
        ...
        if (_mean > 500 && _std < _mean * 0.2) { \
            bias_start = (start); bias_count = MIN_REST_FRAMES; stable_found = 1; \
        } \
    } while(0)
    ...
    /* 如果仍然没找到, 使用5秒处 */
    if (!stable_found) {
        bias_start = MAX(0, ref_start - MIN_REST_FRAMES);
        bias_count = ref_start - bias_start;
        if (bias_count < 1) {
            bias_start = ref_start;
            bias_count = 1;
        }
    }
    if (bias_start + bias_count > row) bias_count = row - bias_start;
    ...
    for (int i = bias_start; i < bias_start + bias_count; i++) {
        bias_wx += raw_wx[i];
        bias_wy += raw_wy[i];
        bias_wz += raw_wz[i];
    }
...
void detect_movement_phases(IMU_Record* data, int len, MovementPhases* phases)
{
    ...
    int hold_end = motion_end;
    int lower_end = motion_end;
    ...
    printf("[阶段检测] 静息结束=%.2fs 抬举=%.2fs-%.2fs 保持=%.2fs-%.2fs(%.1fs)\n",
...
void calc_arm_angle(IMU_Record* data, int len, const MovementPhases* phases)
{
    ...
    if (ref_c < 1 || ref_s < 0 || ref_s >= len) { free(ax_f); free(ay_f); free(az_f); return; }
    ...
    if (actual_ref_c < 1) { free(ax_f); free(ay_f); free(az_f); return; }

    ref_gx /= actual_ref_c; ref_gy /= actual_ref_c; ref_gz /= actual_ref_c;
                     ...
                     const MovementPhases* phases, double* rmsj)
{
    *rmsj = 6000.0;  /* 默认最差值 (deg/s^3) */
    ...
    for (int i = 0; i < act_len; i++) {
        ...
        vel_mag[i] = data[idx].ang_vel_mag;
    }
                 ...
                 double* tremor_ratio, double* pd_tremor_ratio)
{
    ...
    *pd_tremor_ratio = 0.3;
    ...
    /* ---- 主要指标: 保持期角速度标准差 ---- */
    double wstd = detrended_ang_vel_std(data, start, end);
    ...
    for (int axis = 0; axis < 3; axis++) {
        ...
        mean_val /= act_len;
        for (int i = 0; i < act_len; i++) {
            double window = (act_len > 1) ?
                (0.54 - 0.46 * cos(2.0 * PI * i / (act_len - 1))) : 1.0;
            signal[i] = (signal[i] - mean_val) * window;
        }
                ...
                double* completion_time, double* peak_ang_vel)
{
    ...
    *peak_ang_vel = percentile_ang_vel_mag(data, start, end + 1, 0.95);
}
...
void run_single_hand_assessment(const char* csv_filename)
{
    ...
    if (phases.valid) {
        printf("  静息结束: %.2fs | 抬举: %.2fs-%.2fs | 保持: %.2fs-%.2fs\n",
    ...
    /* ---- 追加一行到评估日志CSV ---- */
    {
        ...
        FILE *rf = fopen(get_assessment_log_path(), "r");
        int need_header = (rf == NULL);
        if (rf) fclose(rf);

        rf = fopen(get_assessment_log_path(), "a");
        if (rf) {
            if (need_header) {
...
static double shifted_best_corr(const double* x, const double* y, int n, int max_shift)
{
    double best = -1.0;
    ...
    for (int shift = -max_shift; shift <= max_shift; shift++) {
        ...
        if (count < 5) continue;
        double c = pearson_corr2(px, py, count);
        if (c > best) best = c;
    }
...
static double ratio01(double a, double b)
{
                             ...
                             double* temporal_ratio, double* peak_vel_ratio)
{
    ...
    *peak_vel_ratio  = 0.0;
    ...
    if (len_rise_L < 5 || len_rise_R < 5) return;
    ...
    if (common_len < 40) common_len = 40;
    if (common_len > 300) common_len = 300;
    ...
    int max_shift = common_len / 10;
    *vel_corr = shifted_best_corr(resampL, resampR, common_len, max_shift);
    ...
    double peakL = percentile_array(velL, len_rise_L, 0.95);
    double peakR = percentile_array(velR, len_rise_R, 0.95);
    *peak_vel_ratio = ratio01(peakL, peakR);

    double angleL = 0.0, angleR = 0.0;
    for (int i = rL_start; i < phasesL->hold_end && i < lenL; i++) {
        if (left_data[i].arm_angle > angleL) angleL = left_data[i].arm_angle;
    }
    for (int i = rR_start; i < phasesR->hold_end && i < lenR; i++) {
        if (right_data[i].arm_angle > angleR) angleR = right_data[i].arm_angle;
    }
    double angle_ratio = ratio01(angleL, angleR);

    double corr_dev  = 1.0 - (*vel_corr);
    double temp_dev  = fabs(*temporal_ratio - 0.5) / 0.5;
    double peak_dev  = 1.0 - (*peak_vel_ratio);
    double angle_dev = 1.0 - angle_ratio;

    if (temp_dev > 1.0) temp_dev = 1.0;
    if (peak_dev > 1.0) peak_dev = 1.0;
    if (angle_dev > 1.0) angle_dev = 1.0;

    *asymmetry_index = 0.35 * corr_dev +
                       0.25 * temp_dev +
                       0.25 * peak_dev +
                       0.15 * angle_dev;
