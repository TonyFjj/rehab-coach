...
#define PREP_FRAMES 480      // 准备阶段帧数 (5秒, 此期间不分析)

/* 评估日志CSV文件名 (运行时自动放到项目data目录) */
#define ASSESSMENT_LOG_NAME "assessment_log.csv"
...
typedef struct
{
    ...
    int hold_end;    // 保持阶段结束
    int lower_start; // 兼容字段: 当前方案不检测放下, 等于保持结束
    int lower_end;   // 兼容字段: 当前方案不检测放下, 等于采集结束
